...
class Trainer(object):
    def __init__(self, arg):
        ...
        self.Matcher = Matcher(self.embed_dim, self.num_symbols,
                               ...
                               device=self.device,lam = self.lam, 
                               BiLSTM_hidden_size = self.BiLSTM_hidden_size,
                               Bilstm_num_layers = self.Bilstm_num_layers,
                               Bilstm_seq_length = self.Bilstm_seq_length,
                               BiLSTM_input_size = self.BiLSTM_input_size,
                               max_rel = self.max_rel, max_tail = self.max_tail
                               )
        ...
        degrees = self.build_connection(max_rel=self.max_rel, max_tail = self.max_tail)
    ...
    def build_connection(self, max_rel=10, max_tail = 10):
        self.connections = (np.ones((self.num_ents, max_rel, max_tail, 2)) * self.pad_id).astype(int)
        ...
        for ent, id_ in self.ent2id.items():
            ...
            if len(neighbors) > max_rel * max_tail:
                neighbors = neighbors[:max_rel * max_tail]
            ...
            for idx, _ in enumerate(neighbors):
                if(_[0] not in set_nbr_rel):
                    set_nbr_rel.add(_[0])
                    index_nums+=1
                    index = 0
                if(index_nums == max_rel + 1):
                    break
                if(index==max_tail):
                    continue
                self.connections[id_,index_nums-1, index, 0] = _[0]  # rel
                self.connections[id_,index_nums-1, index, 1] = _[1]  # tail
                index+=1
        return degrees
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,
            ...
            #support:(5,2)  query(128,2) false(128,2)
            self.batch_nums += 1
            ...
            self.Matcher.train()
            if self.no_meta:
                positive_score, negative_score, CL = self.Matcher(support, query, false, isEval=False)
            else:
                positive_score, negative_score, CL = self.Matcher(support, query, false, isEval=False,
            ...
            loss = F.relu(self.margin - margin_).mean() + CL
            ...
            if self.batch_nums - best_batches > self.eval_every * 30:
    ...
    def eval(self, mode='dev', meta=False):
        ...
        for query_ in test_tasks.keys():
            ...
            for triple in test_tasks[query_][few:]:
                ...
                if meta:
                    query_meta = self.get_meta(query_left, query_right)
                    scores, _, _ = self.Matcher(support, query, None, isEval=True,
                                             support_meta=support_meta,
                                             query_meta=query_meta,
                                             false_meta=None)
                    scores.detach()
